...
<head>
    ...
    <title>Register - {{ $course['title'] }} | Goshen Work Skill Association</title>
...
<body class="bg-[#fcfcfc]">
    ...
    <section class="relative py-12 md:py-16 bg-gradient-to-r from-[#091c3d] to-[#0f172a] text-white overflow-hidden">
        <div class="relative z-10 max-w-4xl mx-auto px-4 text-center">
            <span class="bg-[#c1121f] text-[10px] md:text-xs font-bold uppercase tracking-wider px-3 py-1 rounded-full mb-3 inline-block">{{ $course['category'] }}</span>
            <h1 class="font-serif text-2xl md:text-4xl font-bold mb-2">{{ $course['title'] }}</h1>
            <p class="text-gray-300 text-sm">Complete the form below and pay the subscription fee to activate your student account.</p>
        </div>
    </section>
                ...
                <div class="bg-white rounded-2xl shadow-lg border border-gray-100 p-6 md:p-8">
                    ...
                    <form method="POST">
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                <h3 class="font-bold text-[#091c3d] text-sm mb-4 border-b border-gray-100 pb-2">
                                    <i class="fa-solid fa-address-book text-[#c1121f] mr-2"></i> Contact &amp; Education
                                </h3>
                                <div class="grid md:grid-cols-2 gap-4">
                                    <div class="md:col-span-2">
                                        <label class="block text-xs font-semibold text-gray-700 mb-1">Address <span class="text-gray-400">(optional)</span></label>
                                        <textarea name="address" rows="2" class="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-lg focus:border-[#c1121f] outline-none">{{ old('address') }}</textarea>
                                    </div>
                                    <div>
                                        <label class="block text-xs font-semibold text-gray-700 mb-1">Emergency Contact Name <span class="text-gray-400">(optional)</span></label>
                                        <input type="text" name="emergency_contact_name" value="{{ old('emergency_contact_name') }}"
                                            class="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-lg focus:border-[#c1121f] outline-none">
                                    </div>
                                    <div>
                                        <label class="block text-xs font-semibold text-gray-700 mb-1">Emergency Contact Phone <span class="text-gray-400">(optional)</span></label>
                                        <input type="tel" name="emergency_contact_phone" value="{{ old('emergency_contact_phone') }}"
                                            class="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-lg focus:border-[#c1121f] outline-none">
                                    </div>
                                    <div>
                                        <label class="block text-xs font-semibold text-gray-700 mb-1">Education Level <span class="text-gray-400">(optional)</span></label>
                                        <select name="education_level"
                                            ...
                                            <option value="Degree" {{ old('education_level') === 'Degree' ? 'selected' : '' }}>Bachelor's Degree</option>
                                ...
                                <div class="grid md:grid-cols-2 gap-4">
                                    ...
                                    <div>
                                        ...
                                        <input type="text" name="transaction_reference" value="{{ old('transaction_reference') }}" required
                                            placeholder="MoMo ref or bank transfer ref"
                                            class="w-full px-4 py-2.5 text-sm border border-gray-200 rounded-lg focus:border-[#c1121f] outline-none">
...
--}}
                    <div class="border-t border-gray-100 pt-4 space-y-3 text-xs text-gray-600">
                        <div class="flex justify-between">
                            <span>Course</span>
                            <span class="font-bold text-[#091c3d] text-right">{{ $course['title'] }}</span>
                        </div>
                        <div class="flex justify-between">
                            <span>Duration</span>
                        ...
                        <div class="flex justify-between">
                            ...
                            <span class="font-bold text-[#091c3d] text-right">{{ $course['summary']['certification'] }}</span>
                        </div>
                    </div>
